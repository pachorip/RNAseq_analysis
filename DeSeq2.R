...
head(countdata)
    colData<-read.csv("experiment.csv",header=TRUE,row.names=1)
condition <- colData[,1]
head(condition)
...
library(gplots)
png("qc-heatmap-samples.png", w=1000, h=1000, pointsize=15)
...
rld_pca <- function (rld, intgroup = "condition", ntop = 500, colors=NULL, legendpos="bottomleft", main="PCA Biplot", textcx=1, ...) {
  ...
  #  textcx;(PC2 ~ PC1, groups = fac, data = as.data.frame(pca$rld),
...
res <- results(dds, contrasts=c("condition","ApoLF","hLF"))
#res <- results(dds)
...
### Test for DE up and down regulated genes
...
write.csv(mat, file="diffexpr-heatmapdata.csv")
png("top50genes.png", 1800, 1200, pointsize=20)
pheatmap(mat,fontsize = 20)
dev.off()
...
if (labelsig) {
 require(calibrate)
  with(subset(res, padj<thresh), textxy(baseMean, log2FoldChange, labs=FALSE, cex=0.7, col=2))
 }
}
...
volcanoplot <- function (res, lfcthresh=2, sigthresh=0.05, main="Volcano Plot", legendpos="bottomright", labelsig=TRUE, textcx=1, ...) {
  ...
  if (labelsig) {
    require(calibrate)
    with(subset(res, padj<sigthresh & abs(log2FoldChange)>lfcthresh), textxy(log2FoldChange, -log10(pvalue), labs=FALSE, cex=textcx, ...))
  }
